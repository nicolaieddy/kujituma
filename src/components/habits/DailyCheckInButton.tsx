...
import { Sun, Check, CloudOff } from "lucide-react";
...
export const DailyCheckInButton = () => {
  ...
  const { hasCheckedInToday, isLoading, todayCheckIn } = useDailyCheckIn();
  const pendingIds = usePendingSyncIds("daily_check_ins");
  const isPendingSync =
    !!todayCheckIn && (pendingIds.has((todayCheckIn as any).id) || !navigator.onLine && hasCheckedInToday);
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <div className="relative">
              ...
              {isPendingSync && (
                <span className="absolute -top-1 -right-1 inline-flex h-5 w-5 items-center justify-center rounded-full border-2 border-background bg-amber-500 text-white shadow-sm">
