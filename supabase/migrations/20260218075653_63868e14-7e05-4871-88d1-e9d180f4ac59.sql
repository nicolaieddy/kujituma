...
BEGIN
  ...
  -- Create notification for partner (correct argument order: user_id, type, message, related_post_id, related_comment_id, triggered_by_user_id, related_request_id)
  PERFORM create_notification(
    v_partner_id,           -- _user_id (recipient)
    'accountability_check_in', -- _type
    v_notification_message, -- _message
    NULL,                   -- _related_post_id
    NULL,                   -- _related_comment_id
    v_user_id,              -- _triggered_by_user_id (the person who did the check-in)
    NULL                    -- _related_request_id
  );
