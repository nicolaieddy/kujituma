-- Update get_partner_dashboard_data to include objective counts per goal
-- This adds a historical_objectives_count to each goal
    ...
    'goals', CASE WHEN v_can_view THEN (
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        'completed_objectives_count', (
          SELECT COUNT(*)::int 
          FROM weekly_objectives wo 
          WHERE wo.goal_id = g.id AND wo.is_completed = true
        )
